...
Ради киллерфичи, тем более я считаю, что эта киллер фича отличает мою библиотеку от всех других. А киллефича, это проверка графа зависимостей на правильность. Это набор разных проверок, которые позволяют избежать проблем во время исполнениях, что экономит время разработки. И проверка на циклы выделяется отдельно среди этих проверок, так как эта операция занимает гораздо больше времени, и до недавнего времени на среднем проекте уже становилась бесполезной, так как дождаться завершения можно было только к старости.

О проблеме я знал давно, но понимал, что в том виде в каком сейчас хранится граф сделать быструю проверку сложно. Да и раз уж библиотека умеет проверять граф зависимостей, то сделать "Graph API" само напрашивается. Graph API - позволяет отдавать граф зависимостей для внешнего пользования, для того чтобы:
...
* И проверка графа на корректность.
...
Но прежде чем напрягать мозги, пальцы уже сделали своё дело, и пока я думал какой час, на экране уже успел появится код для создания графа в представлении матрице смежности. Жалко конечно но код написанный не в веденье пришлось переписать - для моей задачи важнее как можно быстрее находить исходящие ребра, а вот входящие меня мало интерисуют. Да и память в наше время безгранична - чтобы её не сэкономить?
...
```
Запустил этот алгоритм, подождал 10 минут... и конечно же ушел спать - 6 утра же на часах...

Пока спал думал - а почему так долго? Про размер графа я уже писал, но в чем проблема данного алгоритма? Проблема в том, что для многих вершин количество вызовов функции dfs сотавляет миллион, а у некоторых по 30 миллионов раз. То есть в среднем 900 вершин * 1000000 = 900.000.000 вызовов функции dfs... 

Откуда такие бешенные цифры? Да будь бы это обычный [лес](https://ru.wikipedia.org/wiki/%D0%94%D0%B5%D1%80%D0%B5%D0%B2%D0%BE_(%D1%82%D0%B5%D0%BE%D1%80%D0%B8%D1%8F_%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2)#:~:text=%D0%9B%D0%B5%D1%81%20%E2%80%94%20%D0%BC%D0%BD%D0%BE%D0%B6%D0%B5%D1%81%D1%82%D0%B2%D0%BE%20%D0%B4%D0%B5%D1%80%D0%B5%D0%B2%D1%8C%D0%B5%D0%B2.,%D0%B2%D0%B5%D0%B4%D1%91%D1%82%20%D1%80%D0%BE%D0%B2%D0%BD%D0%BE%20%D0%BF%D0%BE%20%D0%BE%D0%B4%D0%BD%D0%BE%D0%B9%20%D0%B4%D1%83%D0%B3%D0%B5).), то все бы быстро отработало, но у нас же граф с циклами... zzzzzz...

# Этап 3. Наивная оптимизация
Проснулся я снова не по завету после обеда, и первым делом было не пойти в туалет, и уж точно не поесть, а посмотреть сколько же выполнялнся мой алгоритм всего, а ну да всегото полтора часа... Ну ладно я то за ночь придумал как оптимизировать!
В той реализации ищутся циклы только проходящие через указанную вершину, а все подциклы найденные при поиске основного цикла просто игнорируются. Возникает желание перестать их игнорировать - ведь тогда получается, что можно выкидывать из рассмотрения все вершины через которые мы уже прошли. Сделать это не так сложно, правда когда пальци не попадают на клавиатуру чуть сложнее, но каких-то пол часа и готово:
```
func findAllCycles() -> [Cycle] {
...
Интуиция мне подсказывала, что первый вариант лучше. На самом деле он понятней для анализа, так как мы запоминаем циклы для конкретной вершины, а не для всего подрят.
...
Ладно еще целых пол дня впереди. На самом деле пол. дня мне не было нужно, так как посмотрев картинки и различные данные которые я выводил о поиске, стало понятно, что есть ситуации где функция dfs вызывается 30миллионов раз, но находится всего 1-2 цикла. Такое возможно в случаях на подобии:
...
К сожалению текст для статьи я писал гораздо дольше, чем код для реализации этого:
```
func findAllCycles() -> [Cycle] {
...
Все время пока я писал этот алгоритм, и то время пока спал, меня мучал вопрос - а можно ли как-то использовать результат прошлых вычислений. Ну ведь я уже нашел все циклы через вершину A, наверное что-то это да значит для других циклов.
...
И только выпив целиком весь стакан с водой, а он 250мл к сведенью, я осознал, что эту проверку можно вставить прям внутри поиска в глубину:
```
func findAllCycles() -> [Cycle] {
	reachableIndices: [Set<Int>] = findAllReachableIndices()
	result: [Cycle] = []
...
```
Посмотрев на это простое решение, я нажал run, и был уже готов снова отойти от компьютера, как вдруг - все проверки прошли... 6 секунд? Не не может быть... Да не по аналитическим данным все циклы были найдены.
...
# Этап 6. Профайл
Результат в 5-6 секунд меня уже устраивал, но хотелось еще быстрее, да еще даже не вечер! Поэтому я открыл профайл. Я понимал что на языке Swift низкоуровневая оптимизация почти невозможна, но иногда находишь проблемы в неожиданных местах.
...
Запустив приложение и увидев 3 секунды, я уже хотел было остановиться, но меня мучало одно предчувствие по поводу обхода в глубину. Я давно знал, что массивы у apple сделаны так, что вставка вначало и вконец массива занимает константное время в силу кольцевой реализации внутри (извиняюсь, я не помню как правильно это называется). И так-же на языке Swift у массива есть интересная функция `popLast()`, но нет аналога для первого элемента. Но проще показать.
было (язык Swift):
```Swift
var visited: Set<Int> = []
...
return visited
```
Вроде изменения не значительные, и кажется, что второй код должен работать медленнее - и на многих языках второй код будет работать медленнее, но на swift он быстрее на 5-10% оказался.
...
Ну а появлению статьи благодарим гугл - который не захотел мне помогать, и я придумывал все из головы.
...
P.S. Если отключить 5 этап полностью, это который добавил доп. действие перед началом поиска, то скорость работы понизится в 1.5 раза - до 4.5 секунд. То есть в этой операции даже после всех других оптимизаций есть толк.
